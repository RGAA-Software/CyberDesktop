...
```text
crates/editor-text-engine/   纯逻辑、无 GPUI：buffer / encoding / loader / selection / history / document / search / syntax
crates/editor-ui/     GPUI 自绘编辑 Element + 输入 + 标签/状态栏/查找面板（外壳继续用 gpui-component）
```
...
- [x] `crates/editor-text-engine` 脚手架 + 接入 workspace
...
```powershell
cargo test -p editor-text-engine
cargo build -p editor-app --bin cyber_editor
cargo run  -p editor-app --bin cyber_editor -- <file>
```
