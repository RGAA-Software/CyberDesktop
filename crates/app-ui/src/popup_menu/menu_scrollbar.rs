...
pub const MENU_SCROLLBAR_GAP: Pixels = px(4.);
const WIDTH: Pixels = MENU_SCROLLBAR_WIDTH;
const MIN_THUMB_SIZE: f32 = 48.;

const THUMB_WIDTH: Pixels = px(5.);
const THUMB_RADIUS: Pixels = px(2.5);
const THUMB_INSET: Pixels = px(2.5);

const THUMB_ACTIVE_WIDTH: Pixels = px(5.);
const THUMB_ACTIVE_RADIUS: Pixels = px(2.5);
const THUMB_ACTIVE_INSET: Pixels = px(2.5);

const FADE_OUT_DURATION: f32 = 3.0;
const FADE_OUT_DELAY: f32 = 2.0;

#[inline]
fn scrollbar_show_is_hover(mode: ScrollbarShow) -> bool {
    matches!(mode, ScrollbarShow::Hover)
}

#[inline]
fn scrollbar_show_is_always(mode: ScrollbarShow) -> bool {
    matches!(mode, ScrollbarShow::Always)
}
...
/// Narrow scrollbar used only by [`super::PopupMenu`].
...
impl IntoElement for MenuScrollbar {
    type Element = Self;
...
impl Element for MenuScrollbar {
    ...
    fn prepaint(
        ...
        for axis in self.axis.all().into_iter() {
            ...
            let scrollbar_show = self.scrollbar_show.unwrap_or(cx.theme().scrollbar_show);
            let is_always_to_show = scrollbar_show_is_always(scrollbar_show);
