...
impl EditorSession {
    ...
    pub(crate) fn set_dirty(&mut self, dirty: bool) -> bool {
        let changed = self.dirty != dirty;
        self.dirty = dirty;
        changed
    }

    pub(crate) fn refresh_metadata_from_text(&mut self, current_text: &str) -> bool {
        let next_line_ending = detect_line_ending(current_text);
        let next_indent_style = detect_indent_style(current_text);
        let changed =
            self.line_ending != next_line_ending || self.indent_style != next_indent_style;
        self.line_ending = next_line_ending;
        self.indent_style = next_indent_style;
        changed
    }
