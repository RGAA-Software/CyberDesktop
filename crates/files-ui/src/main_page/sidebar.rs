...
impl MainPage {
    ...
    /// Reload every file browser (e.g. after tag assignment changes list badges).
    pub fn reload_all_file_browsers(&mut self, cx: &mut Context<Self>) {
        for tab in &self.tabs {
            let shell = tab.shell.clone();
            shell.update(cx, |shell, cx| {
                shell.for_each_pane(|pane| {
                    pane.update(cx, |pane, cx| {
                        if !matches!(pane.target(), NavigationTarget::Home) {
                            pane.file_browser().update(cx, |browser, cx| {
                                browser.reload(cx);
                                cx.notify();
                            });
