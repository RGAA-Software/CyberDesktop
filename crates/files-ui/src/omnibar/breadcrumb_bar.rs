...
pub struct PathBreadcrumbBar {
    ...
    on_drop_external_paths: Rc<dyn Fn(PathBuf, Vec<PathBuf>, &mut Window, &mut App)>,
    on_drag_hover: Rc<dyn Fn(PathBuf, Vec<PathBuf>, &mut Window, &mut App)>,
    on_drag_hover_external: Rc<dyn Fn(PathBuf, Vec<PathBuf>, &mut Window, &mut App)>,
...
impl RenderOnce for PathBreadcrumbBar {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let on_drop_external_paths = self.on_drop_external_paths.clone();
        let on_drag_hover = self.on_drag_hover.clone();
        let on_drag_hover_external = self.on_drag_hover_external.clone();
        ...
        for &index in &layout.visible_indices {
            ...
            bar = bar.child(render_path_segment(
                ...
                on_drop_paths.clone(),
                on_drop_external_paths.clone(),
                on_drag_hover.clone(),
                on_drag_hover_external.clone(),
                cx,
            ));
        }
...
fn render_path_segment(
    ...
    let drop_external = on_drop_external_paths.clone();
    let hover = on_drag_hover.clone();
    let hover_external = on_drag_hover_external.clone();
    ...
    if is_dir {
        ...
        segment = segment.on_drag_move::<ExternalPaths>({
            let hover = hover_external.clone();
            let hover_target = hover_target.clone();
            let hover_bounds = hover_bounds.clone();
    ...
    if is_dir && !is_last {
        ...
        segment = segment.on_drop({
            let drop = drop_external.clone();
            let drop_target = drop_target.clone();
            move |paths: &ExternalPaths, window, cx| {
                drop(drop_target.clone(), paths.paths().to_vec(), window, cx);
            }
        });
    } else if is_dir {
        segment = segment.on_drop({
