...
use files_core::{self, save_config, AppConfig, WINDOW_HEIGHT, WINDOW_WIDTH};
...
fn persist_preferences(cx: &mut App) {
    let (window_width, window_height) = window_size_for_persist(cx).unwrap_or_else(|| {
...
/// Reads the main CyberFiles window size for persistence.
pub fn window_size_for_persist(cx: &mut App) -> Option<(f32, f32)> {
    #[cfg(feature = "full-app")]
    {
        if let Some(size) = crate::app_state::MainWindowState::window_size(cx) {
            return Some(size);
        }
    }
    window_size_from_active(cx)
}
...
pub fn persist_window_bounds(cx: &mut App) {
    let Some((window_width, window_height)) = window_size_for_persist(cx) else {
        return;
    };
    let _ = save_config(&capture_config(cx, window_width, window_height));
}
...
fn mutate_config(cx: &mut App, mutate: impl FnOnce(&mut AppConfig)) {
    let mut config = files_core::load_config().unwrap_or_default();
    mutate(&mut config);
...
pub fn add_file_tag(name: SharedString, cx: &mut App) {
    ...
    mutate_config(cx, |c| {
        ...
        let used: Vec<Option<String>> = c.file_tags.iter().map(|tag| tag.color.clone()).collect();
        let color = files_fs::pick_random_tag_color(&used);
        c.file_tags.push(files_core::FileTagConfig {
            name: trimmed,
            color: Some(color),
            paths: Vec::new(),
        });
...
pub fn toggle_paths_in_file_tag(tag_name: &str, paths: &[PathBuf], cx: &mut App) {
    if paths.is_empty() {
        return;
    }
    let assigned = file_tags_containing_paths(paths);
    if assigned.iter().any(|name| name == tag_name) {
        remove_paths_from_file_tag(tag_name, paths, cx);
    } else {
        assign_paths_to_file_tag(tag_name, paths, cx);
    }
}

pub fn set_file_tag_color(tag_name: &str, color: Option<String>, cx: &mut App) {
    mutate_config(cx, |c| {
        if let Some(tag) = c.file_tags.iter_mut().find(|t| t.name == tag_name) {
            tag.color = color;
        }
    });
    refresh_tag_ui(cx);
...
fn refresh_all_file_browsers(cx: &mut App) {
    if let Some(nav) = cx.try_global::<crate::app_state::AppNavigation>() {
        let page = nav.main_page();
        let _ = page.update(cx, |page, cx| {
            page.refresh_sidebar_cache(cx);
            page.reload_all_file_browsers(cx);
        });
    }
}
...
pub fn context_menu_show_extract(_cx: &App) -> bool {
    files_core::load_config()
        .map(|c| c.context_menu_show_extract)
        .unwrap_or(true)
}

pub fn apply_context_menu_show_extract(enabled: bool, cx: &mut App) {
    mutate_config(cx, |c| c.context_menu_show_extract = enabled);
}
...
pub fn open_text_with_cybereditor(_cx: &App) -> bool {
    files_core::open_text_with_cybereditor_enabled()
}
...
pub fn open_media_with_cybermediaplayer(_cx: &App) -> bool {
    files_core::open_media_with_cybermediaplayer_enabled()
}

pub fn apply_open_media_with_cybermediaplayer(enabled: bool, cx: &mut App) {
...
pub fn apply_auto_restore_tabs(enabled: bool, cx: &mut App) {
...
pub fn capture_config(cx: &App, window_width: f32, window_height: f32) -> AppConfig {
    let prior = files_core::load_config().unwrap_or_default();
    AppConfig {
        ...
        file_group_option: prior.file_group_option,
        file_group_date_unit: prior.file_group_date_unit,
        ...
        context_menu_show_extract: prior.context_menu_show_extract,
