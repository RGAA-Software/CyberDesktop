...
impl EngineEditor {
    ...
    pub(crate) fn render_hscrollbar(&self, cx: &mut Context<Self>) -> Option<Stateful<gpui::Div>> {
        ...
        let hscrollbar_right = if self.show_preview { self.preview_width + px(14.0) } else { px(14.0) };
        Some(
            div()
    ...
    pub(crate) fn render_scrollbar(&self, cx: &mut Context<Self>) -> Option<Stateful<gpui::Div>> {
        ...
        let scrollbar_right = if self.show_preview { self.preview_width } else { px(0.0) };
        Some(
            div()
                .id("editor-scrollbar")
                .absolute()
