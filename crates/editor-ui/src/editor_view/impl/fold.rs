...
use editor_text_engine::{build_display_lines, crease_at_line, fold_header, FoldRange};

use super::super::canvas::fold_hit_bounds;
...
impl EngineEditor {
    ...
    pub(crate) fn fold_gutter_hit(&self, pos: Point<Pixels>) -> Option<usize> {
        ...
        let vl = self.visible.iter().find(|vl| {
            let cell = fold_hit_bounds(fold_left, vl.top, self.line_height);
            pos.x >= cell.left()
                && pos.x < cell.right()
                && pos.y >= cell.top()
                && pos.y < cell.bottom()
