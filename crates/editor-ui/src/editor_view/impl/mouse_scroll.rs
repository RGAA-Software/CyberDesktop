...
impl EngineEditor {
    pub(crate) fn index_for_position(&self, pos: Point<Pixels>) -> usize {
        ...
        let expanded = expand_tabs(&vl.fragment_text, EDITOR_TAB_SIZE);
        let char_in_fragment = expanded.expanded_byte_to_original_char(byte);
    ...
    pub(crate) fn wrap_index_for_position(
        ...
        if wl.fragment_text.is_empty() {
            ...
            let char_in_line = expanded.expanded_byte_to_original_char(byte);
            wl.start_char + char_in_line
        } else {
            let expanded = expand_tabs(&wl.fragment_text, EDITOR_TAB_SIZE);
            let char_in_frag = expanded.expanded_byte_to_original_char(byte);
    ...
    pub(crate) fn on_mouse_move(
        ...
        let hover = self.fold_gutter_hit(event.position);
        if self.fold_gutter_hover_line != hover {
            self.fold_gutter_hover_line = hover;
            cx.notify();
        }
    }
