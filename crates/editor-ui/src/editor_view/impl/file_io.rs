...
use editor_text_engine::{load_file_with_progress, LoadProgress};
...
impl EngineEditor {
    ...
    pub(crate) fn install_loaded(
        &mut self,
        target: usize,
        loaded: editor_text_engine::LoadedFile,
        path: PathBuf,
        restore: Option<(usize, Pixels)>,
...
pub(crate) fn is_droppable_editor_path(path: &Path) -> bool {
    path.is_file() && editor_text_engine::is_cybereditor_openable(path)
}
