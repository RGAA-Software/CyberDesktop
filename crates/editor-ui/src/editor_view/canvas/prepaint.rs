...
pub(crate) fn prepaint_normal(
    ...
    if editor.reveal_caret {
        ...
        let caret_x = if line_len > LONG_LINE_COL_THRESHOLD {
            ...
            let expanded = expand_tabs(&cline, EDITOR_TAB_SIZE);
            let crun = TextRun {
                len: expanded.text.len(),
                font: font.clone(),
                color: default_color,
            ...
            let cshaped = window.text_system().shape_line(
                SharedString::from(expanded.text.clone()),
                font_size,
                &[crun],
                None,
            );
            cshaped.x_for_index(expanded.original_char_to_expanded_byte(cpos.column))
        };
        let margin = px(24.0);
        if caret_x < scroll_x {
            scroll_x = caret_x;
    ...
    for dix in first_display..last_display {
        ...
        if collapsed || editor.crease_at(line).is_some() {
            fold_gutter.push((top, collapsed, line));
        }
