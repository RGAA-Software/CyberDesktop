...
impl LineWidthCache {
    ...
    pub fn line_width_px(
        ...
        let w = if is_long {
            ...
            f32::from(shape_plain_width(window, font, font_size, &text, default_color))
        };
    ...
    pub fn col_x_px(
        ...
        if line_len <= LONG_LINE_COL_THRESHOLD {
            let text = buf.line_text(line);
            let shaped = shape_plain(window, font, font_size, &text, default_color);
            return shaped.x_for_index(char_to_byte(&text, col));
        }
