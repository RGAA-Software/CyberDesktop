...
pub(crate) fn prepaint_wrapped(
    ...
    while y < content_bottom && line < line_count {
        ...
        let (line_text, wrapped, paint_top, start_col, wrap_row_count) = if is_long {
            ...
            let runs = build_runs(syntax, buf, &fragment, frag_byte, font, default_color);
            let Some(wrapped) = shape_one_wrapped(window, &fragment, &runs, font_size, view_w) else {
                bottom_line = line;
                y += block;
                line += 1;
                continue;
            ...
            let Some(wrapped) = shape_one_wrapped(window, &line_text, &runs, font_size, view_w) else {
                bottom_line = line;
                y += block;
                line += 1;
                continue;
        ...
        for cur in cursors {
            ...
            if !cur.is_empty() && range.start <= line_end_char && range.end > line_start_char {
                ...
                let s_byte = char_to_byte(&line_text, local_s);
                let e_byte = char_to_byte(&line_text, local_e);
                let p0 = wrapped
                ...
                && cur.head <= line_end_char
            {
                ...
                let b = char_to_byte(&line_text, local_col);
