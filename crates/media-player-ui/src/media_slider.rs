...
use gpui::{
    ...
    Bounds, Context, Corners, DefiniteLength, DragMoveEvent, Empty, Entity, EntityId, EventEmitter,
    Hsla, InteractiveElement, IntoElement, IsZero, MouseButton, MouseDownEvent, ParentElement as _,
...
///
/// The default value is `MediaSliderValue::Single(0.0)`.
#[derive(Clone, Copy, Debug, PartialEq)]
pub enum MediaSliderValue {
    Single(f32),
    Range(f32, f32),
}

impl std::fmt::Display for MediaSliderValue {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            MediaSliderValue::Single(value) => write!(f, "{}", value),
            MediaSliderValue::Range(start, end) => write!(f, "{}..{}", start, end),
        }
    }
}

impl From<f32> for MediaSliderValue {
    fn from(value: f32) -> Self {
        MediaSliderValue::Single(value)
    }
}

impl From<(f32, f32)> for MediaSliderValue {
    fn from(value: (f32, f32)) -> Self {
        MediaSliderValue::Range(value.0, value.1)
    }
}

impl From<Range<f32>> for MediaSliderValue {
    fn from(value: Range<f32>) -> Self {
        MediaSliderValue::Range(value.start, value.end)
    }
}

impl Default for MediaSliderValue {
    fn default() -> Self {
        MediaSliderValue::Single(0.)
    }
}

impl MediaSliderValue {
    /// Clamp the value to the given range.
    pub fn clamp(self, min: f32, max: f32) -> Self {
        match self {
            MediaSliderValue::Single(value) => MediaSliderValue::Single(value.clamp(min, max)),
            MediaSliderValue::Range(start, end) => {
                MediaSliderValue::Range(start.clamp(min, max), end.clamp(min, max))
            }
        }
    }
    ...
    pub fn is_single(&self) -> bool {
        matches!(self, MediaSliderValue::Single(_))
    }

    /// Check if the value is a range of values.
    #[inline]
    pub fn is_range(&self) -> bool {
        matches!(self, MediaSliderValue::Range(_, _))
    }

    /// Get the start value.
    pub fn start(&self) -> f32 {
        match self {
            MediaSliderValue::Single(value) => *value,
            MediaSliderValue::Range(start, _) => *start,
        }
    }
    ...
    pub fn end(&self) -> f32 {
        match self {
            MediaSliderValue::Single(value) => *value,
            MediaSliderValue::Range(_, end) => *end,
        }
    }

    fn set_start(&mut self, value: f32) {
        if let MediaSliderValue::Range(_, end) = self {
            *self = MediaSliderValue::Range(value.min(*end), *end);
        } else {
            *self = MediaSliderValue::Single(value);
        }
    }

    fn set_end(&mut self, value: f32) {
        if let MediaSliderValue::Range(start, _) = self {
            *self = MediaSliderValue::Range(*start, value.max(*start));
        } else {
            *self = MediaSliderValue::Single(value);
        }
    }
...
pub enum MediaSliderScale {
    ...
    ///
    /// ```
    /// use gpui_component::slider::{MediaSliderState, MediaSliderScale};
    ///
    /// let slider = MediaSliderState::new()
    ///     .min(1.0)    // Must be > 0 for logarithmic scale
    ///     .max(1000.0)
    ///     .scale(MediaSliderScale::Logarithmic);
    /// ```
    ///
...
impl MediaSliderScale {
    #[inline]
    pub fn is_linear(&self) -> bool {
        matches!(self, MediaSliderScale::Linear)
    }

    #[inline]
    pub fn is_logarithmic(&self) -> bool {
        matches!(self, MediaSliderScale::Logarithmic)
    }
}

/// State of the [`MediaSlider`].
pub struct MediaSliderState {
    min: f32,
    max: f32,
    step: f32,
    value: MediaSliderValue,
    ...
    scale: MediaSliderScale,
    /// Tracks whether the user is currently interacting with the slider so we
    /// only emit [`MediaSliderEvent::Release`] after a real press/drag.
    dragging: bool,
}

impl MediaSliderState {
    /// Create a new [`MediaSliderState`].
    pub fn new() -> Self {
        Self {
            min: 0.0,
            max: 100.0,
            step: 1.0,
            value: MediaSliderValue::default(),
            percentage: (0.0..0.0),
            bounds: Bounds::default(),
            scale: MediaSliderScale::default(),
            dragging: false,
        }
    }
    ...
    pub fn min(mut self, min: f32) -> Self {
        if self.scale.is_logarithmic() {
            assert!(
                min > 0.0,
                "`min` must be greater than 0 for MediaSliderScale::Logarithmic"
            );
            assert!(
                min < self.max,
    ...
    /// Set the scale of the slider, default: [`MediaSliderScale::Linear`].
    pub fn scale(mut self, scale: MediaSliderScale) -> Self {
    ...
    pub fn default_value(mut self, value: impl Into<MediaSliderValue>) -> Self {
    ...
    pub fn set_value(
        &mut self,
        value: impl Into<MediaSliderValue>,
        _: &mut Window,
        cx: &mut Context<Self>,
    ) {
        self.value = value.into();
    ...
    pub fn value(&self) -> MediaSliderValue {
        self.value
    }
    ...
    fn percentage_to_value(&self, percentage: f32) -> f32 {
        match self.scale {
            MediaSliderScale::Linear => self.min + (self.max - self.min) * percentage,
            MediaSliderScale::Logarithmic => {
    ...
    fn update_value_by_position(
        ...
        cx.emit(MediaSliderEvent::Change(self.value));
        cx.notify();
    }

    /// Emit [`MediaSliderEvent::Release`] if the user was actively interacting
    ...
    fn handle_release(&mut self, cx: &mut Context<Self>) {
        ...
        self.dragging = false;
        cx.emit(MediaSliderEvent::Release(self.value));
    }
}

impl EventEmitter<MediaSliderEvent> for MediaSliderState {}

/// A custom MediaSlider element with 5px track height.
#[derive(IntoElement)]
pub struct MediaSlider {
    state: Entity<MediaSliderState>,
    axis: Axis,
...
impl MediaSlider {
    /// Create a new [`Slider`] element bind to the [`MediaSliderState`].
    pub fn new(state: &Entity<MediaSliderState>) -> Self {
        Self {
...
impl Styled for MediaSlider {
    fn style(&mut self) -> &mut StyleRefinement {
        &mut self.style
    }
}

impl RenderOnce for MediaSlider {
        ...
        div()
            ...
            .child(
                h_flex()
                    ...
                    .when(axis.is_horizontal(), |this| {
                        this.items_center().h(px(12.)).w_full()
                    })
                    .when(axis.is_vertical(), |this| {
                    ...
                    .child(
                        div()
                            .id("slider-bar")
                            .relative()
                            .when(axis.is_horizontal(), |this| this.w_full().h(px(4.)))
                            .when(axis.is_vertical(), |this| this.h_full().w(px(4.)))
