...
/// C++ grammar extends C, but upstream `tree-sitter-cpp` ships a minimal query
/// (keywords/types/functions only). Merge the C base query so comments, strings,
/// operators, and primitive types are highlighted too.
...
pub(crate) fn language_config(language_id: &str) -> Option<(Language, &'static str)> {
    let (lang, query): (Language, &str) = match language_id {
        "json" | "jsonc" => (
